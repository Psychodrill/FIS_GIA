...
namespace Fbs.Core.Shared
{
    public class DocumentCheck
    {
        ...
        public static List<string> DocNumberCheck(string item)
        {
            ...
            if (withoutPattern.Length == withPattern.Length && withPattern.Length > 10)
            {
                resultError.Add(string.Format("Поле \"Номер документа\" должно быть до 10 символов. Сейчас {0}", withPattern.Length));
            }
