...
insert into Migrations(MigrationVersion, MigrationName) values (21, '021_2012_05_26_CreatingGetAccountAndLogin')
-- =========================================================================
PRINT N'Creating [dbo].[GetAccountAndLogin]...';
GO
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetAccountAndLogin]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetAccountAndLogin]
GO
SET ANSI_NULLS ON
GO
...
begin try
	...
	begin tran	
		...
		begin			
			...
			select @loginNew = @email, @hasFixedIp = isnull(@hasFixedIp, 1), @hasCrocEgeIntegration = isnull(@hasCrocEgeIntegration, 0)
			...
			insert dbo.Account
				(
					...
					Status, IpAddresses, HasFixedIp, HasCrocEgeIntegration
				)
			select GetDate(), GetDate(), @updateId, @editorAccountId, @editorIp, @loginNew, @passwordHash, @lastName, @firstName, @patronymicName, @orgRequestID,
				   ...
				   @hasFixedIp, @hasCrocEgeIntegration
		...
		begin -- update существующего пользователя						
			...
			update account set UpdateDate = GetDate(), UpdateId = @updateId, EditorAccountId = @editorAccountId, PasswordHash=isnull(@passwordHash,PasswordHash), 
							   ...
							   RegistrationDocumentContentType = @registrationDocumentContentType, HasFixedIp = @hasFixedIp, HasCrocEgeIntegration = @hasCrocEgeIntegration,
							   OrganizationId=@orgRequestID,position=@position
