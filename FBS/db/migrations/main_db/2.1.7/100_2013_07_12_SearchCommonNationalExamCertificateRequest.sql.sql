insert into Migrations(MigrationVersion, MigrationName) values (100, '100_2013_07_12_SearchCommonNationalExamCertificateRequest.sql')
go
-- =============================================
...
begin
	...
	begin
		...
		if isnull(@maxRowCount,0) > 1
			set @commandText = @commandText + 
					'
					join @table cc on cc.id=cne_certificate_request.id 
					'						
	...
	set @commandText = '
		declare @table table (pk int identity(1,1), id bigint,primary key(pk,id))
		
		insert @table
		select id from
		(
			select id, row_number() over (order by cc.ParticipantID '+case when @sortAsc = 1 then 'asc' else 'desc' end+') rn
			from 
			(
				select ParticipantID 
				from CommonNationalExamCertificateRequest with (nolock) 
				where @innerBatchId = BatchId
			) cc
			join CommonNationalExamCertificateRequest c on cc.ParticipantID=c.ParticipantID and @innerBatchId = c.BatchId
		) cc
		where rn between '+cast(@startRowIndex as nvarchar(255))+' and '+cast(isnull(@maxRowCount,100000)+@startRowIndex+20 as nvarchar(255)) + ' '
		+@commandText
			...
			set @viewCommandText = @viewCommandText + 
								...
								LEFT JOIN #apl_pvt apl_pvt
									ON 1 = CASE WHEN search.SourceCertificateId = mrk_pvt.CertificateId AND apl_pvt.isCorrect = 1 THEN 1
												...
												ELSE 0
										   END AND mrk_pvt.CertificateId = apl_pvt.CertificateId AND mrk_pvt.ParticipantId1 = apl_pvt.ParticipantId1 
						   '
		...
		begin
			...
			where t.rn between 1 and '+cast(@maxRowCount as nvarchar(255)) + '			
