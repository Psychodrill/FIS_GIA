...
insert into Migrations(MigrationVersion, MigrationName) values (49, '049_2012_06_19_ModifyProcs.sql')
-- =========================================================================
GO

/****** Object:  StoredProcedure [dbo].[CheckCommonNationalExamCertificateByNumberForXml]    Script Date: 06/19/2012 22:21:28 ******/
...
select @checkLastName = null			, @checkFirstName = null, @checkPatronymicName=  null ,@checkTypographicNumber=null
	...
	from @certificate_check certificate_check
		...
		left outer join (
			...
			left join dbo.GetSubjectMarks(@checkSubjectMarks) check_subject	on check_subject.SubjectId = certificate_subject.SubjectId
			left join dbo.[Subject] [subject]	on [subject].Id = check_subject.SubjectId		
			left join [MinimalMark] as mm on check_subject.SubjectId = mm.[SubjectId] and certificate_subject.[Year] = mm.[Year] 
...
IF exists(select * from #table where  SubjectMarkIsCorrect=0) or (select COUNT(*) from #table)<>(select COUNT(*) from dbo.GetSubjectMarks(@checkSubjectMarks))
	delete from #table
