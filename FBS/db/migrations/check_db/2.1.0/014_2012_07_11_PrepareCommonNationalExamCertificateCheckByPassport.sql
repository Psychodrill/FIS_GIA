...
insert into Migrations(MigrationVersion, MigrationName) values (14, '014_2012_07_11_PrepareCommonNationalExamCertificateCheckByPassport')
-- =========================================================================
GO 
...
--—делал так чтобы если есть нет не одного совпадени€ то занул€ть проверку. –азделил на два блока так как не знаб логику работы первого блока
 if not exists(select * from @certificate_subject_correctness_result where ShareCertificateCheckingId=@idcert  and mark is not null and IsSubjectCorrect=1)
