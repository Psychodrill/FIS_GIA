insert into Migrations(MigrationVersion, MigrationName) values (32, '032_2013_07_18_fbs_check_db.sql')
go
    ...
    from 
     (
      select distinct       
        ...
        join <dataDbName>.prn.CertificatesMarks cm with (nolock) on cm.ParticipantFK=exam_certificate_request.ParticipantID and exam_certificate_request.[Year]=cm.UseYear
        left join <dataDbName>.prn.Certificates b with (nolock) on b.CertificateID=cm.CertificateFK and b.[UseYear]=exam_certificate_request.[Year]                                   
          where 1 = case when @IsTypographicNumber = 0 then 1 
                       when exam_certificate_request.TypographicNumber is null and @IsTypographicNumber = 0 then 1 
