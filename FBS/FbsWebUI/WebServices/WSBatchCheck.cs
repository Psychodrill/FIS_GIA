...
namespace Fbs.Web.WebServices
{
    ...
    public class WSBatchCheck : WSBaseCheck
    {
        ...
        public string GetResultNN(string password, string queryXML)
        {
            ...
            var userCredentials = new FbsChecksClient.WSChecksReference.UserCredentials { Login = login, Password = password };
            ...
            using (var scope = new TransactionScope())
            {
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    using (var cmd = new SqlCommand())
                    {
                        ...
                        /* TODO чота все корректные. Надо разобраться с признаком IsCorrect
                        if (!(bool)isCorrect.Value)
                        {
                            AppendStatus(-1, "Некорректен");
                            return false;
                        }
                         */
                ...
                using (var connection = new SqlConnection(connectionString))
                {
                    ...
                    using (var cmd = new SqlCommand())
                    {
                        ...
                        else
                        {
                            ...
                            values (@batchId, @id, @WSSearchType)
                            ";
                            cmd.Parameters.Clear();
                            cmd.Parameters.AddWithValue("@batchId", uniqueId.Value);
                            cmd.Parameters.AddWithValue("@id", id);
                            cmd.Parameters.AddWithValue("@WSSearchType", (int)this.searchType);
                            cmd.ExecuteNonQuery();
                            scope.Complete();
                            return uniqueId.Value;
                        }
