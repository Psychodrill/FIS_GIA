namespace Fbs.Web.Certificates.CommonNationalCertificates
{
    ...
    /// <summary>
    /// The batch check by number.
    /// </summary>
    public partial class BatchCheckByNumber : BasePage
    {
        #region Constants and Fields

        private const string FailedUri = "/Certificates/CommonNationalCertificates/BatchCheckByNumber.aspx";

        private const string SuccessUri =
            "/Certificates/CommonNationalCertificates/BatchCheckByNumberResult.aspx?id={0}";
        ...
        public string GetResult(bool isProcess, object id)
        {
            string enable =
                string.Format(
                    "<a href=\"/Certificates/CommonNationalCertificates/BatchCheckByNumberResult.aspx?id={0}\">результат</a>", 
                    id);
                ...
                if (this.fuData.HasFile)
                {
                    ...
                    using (var reader = new StreamReader(batchStream, Encoding.GetEncoding(1251), true))
                    {
                        ...
                        else
                        {
                            ...
                            var checkClient = new WSCheckClient();
                            checkClient.StartBatchCheck(this.User.Identity.Name, batchText, 1, id, null);
