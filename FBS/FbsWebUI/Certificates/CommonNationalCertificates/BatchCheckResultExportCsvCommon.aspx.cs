...
namespace Fbs.Web.Certificates.CommonNationalCertificates
{
    ...
    /// </summary>
    public partial class BatchCheckResultExportCsvCommon : Page
    {
        #region Methods
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                var exporter = new BatchCheckResultCommonCsvExporter();

                for (int startParticipantIndex = 1; startParticipantIndex <= participantsCount; startParticipantIndex += chunkSize)
                {
                    using (DataTable chunk = svc.GetRecordsPage(startParticipantIndex, chunkSize))
                    {
