<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BatchCheckResult.aspx.cs" 
    Inherits="Fbs.Web.Certificates.CommonNationalCertificates.BatchCheckResult" 
...
<asp:Content ContentPlaceHolderID="cphCertificateContent" runat="server">
 <form id="Form1" runat="server" target="_blank" > 
    ...
    <br>С нормативными документами можно ознакомиться в разделе &laquo;Документы&raquo; Подсистемы ФИС &laquo;Результаты ЕГЭ&raquo;</p>
    </div>
    </div>
    ...
    <asp:DataGrid runat="server" id="dgResultsList"
        ...
        <Columns>
            ...
            <ItemTemplate>
                <%# string.IsNullOrEmpty(Eval("LastName").ToString()) ? Eval("CheckLastName") : Eval("LastName") %>
            </ItemTemplate>
            </asp:TemplateColumn>
            ...
            <ItemTemplate>
                <%#  string.IsNullOrEmpty(Eval("FirstName").ToString()) ? Eval("CheckFirstName") : Eval("FirstName") %>
            </ItemTemplate>
            </asp:TemplateColumn>
            ...
            <ItemTemplate>
                <%# string.IsNullOrEmpty(Eval("PatronymicName").ToString()) ? Eval("CheckPatronymicName") : Eval("PatronymicName") %>
            </ItemTemplate>
            </asp:TemplateColumn>
            ...
            <ItemStyle HorizontalAlign="Center" />
            <HeaderTemplate>
                <div title="Количество уникальных проверок ВУЗами и их филиалами">Пр</div>
            </HeaderTemplate>
            <ItemTemplate>
...
<script runat="server">
   ...
   private static string GetCheckLink(string number, string lastName)
   {
       return
           string.Format(
   ...
   private string ShowCertificateNumber(object dataItem)
   {
       ...
       string numberString = GetCheckLink(number, lastName);
       
       if (!Convert.ToBoolean(dataRow["IsExist"]))
       {
           //string notFoundLink = String.Format("PrintNotFoundNote.aspx?SubjectMarks={0}&number={1}&check=byNumberName&GivenName={2}&FirstName={3}&LastName={4}", HttpUtility.UrlEncode(dataRow.ExtractSubjectsWithMarksString()), number, dataRow["CheckPatronymicName"], dataRow["CheckFirstName"], lastName);
           return
                    string.Format(
       ...
       string newNumberString = GetCheckLink(newNumber, lastName);
