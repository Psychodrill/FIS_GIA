    ...
    public class SqlConstructor_GetUsers : SqlConstructor_GetData
    {
        ...
        protected const string m_mainSQL =
            @"SELECT A.CreateDate, A.[Login], A.LastName, A.FirstName, A.PatronymicName, A.Email,
                O.FullName AS OrganizationName, A.[Status], O.TypeId as TypeId, 
                O.Id as OrgId, ISNULL(A_Op.LastName,'') OperatorName,
                CASE WHEN LEN(ISNULL(OpLog.Comments,''))=0 THEN 1 ELSE 2 END HasComments
              FROM dbo.Account A
              INNER JOIN dbo.Organization2010 O ON A.OrganizationId=O.Id
        ...
        /// The url parameters.
        /// </param>
        public SqlConstructor_GetUsers(NameValueCollection urlParameters)
        {
