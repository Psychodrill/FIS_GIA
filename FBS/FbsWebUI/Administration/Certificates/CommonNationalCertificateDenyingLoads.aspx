<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CommonNationalCertificateDenyingLoads.aspx.cs" 
    Inherits="Fbs.Web.Administration.Certificates.CommonNationalCertificateDenyingLoads" 
...
<asp:Content ContentPlaceHolderID="cphContent" runat="server">
    ...
    <asp:SqlDataSource  runat="server" ID="dsResultsList" 
        ...
        SelectCommand="dbo.SearchCommonNationalExamCertificateDenyLoadingTask"  CancelSelectOnNullParameter="false"
        ...
        object Id, object ErrorCount)
    {
        ...
        if (!Convert.IsDBNull(IsLoaded) && Convert.ToBoolean(IsLoaded))
        {
            if (!Convert.IsDBNull(IsCorrect) && Convert.ToBoolean(IsCorrect))
                ...
                return String.Format("Выполнено (<a href=\"/Administration/Certificates/CommonNationalCertificateDenyingLoadErrors.aspx?id={0}\">{1} ошибок</a>)",
                    Id, ErrorCount);
        }

        if (!Convert.IsDBNull(IsActive) && Convert.ToBoolean(IsActive))
            return String.Format("<a onclick=\"return confirm('Вы действительно хотите аннулировать свидетельство?')\" href=\"/Administration/Certificates/CommonNationalCertificateDenyingLoad.aspx?id={0}\">Выполнить</a>", Id);
