<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BatchCheckFileFormatResult.aspx.cs" 
    Inherits="Fbs.Web.AllUsers.BatchCheckFileFormatResult" 
...
<asp:Content ContentPlaceHolderID="cphCertificateContent" runat="server">
    ...
    <div class="txt-in">
    <p>РЯ - русский язык; М - математика; Ф - физика; Х - химия;
    Б - биология; ИР - история России; Г - география; 
   АЯ - английский язык; НЯ - немецкий язык;>ФЯ - французский язык;
    О - обществознание; Л - литература; ИЯ - испанский язык; 
    И - информатика.</p>
    <p>Коды ошибок:</p>
    <p>
    П - неверное число полей, поля должны разделяться символом "%'; 
    НС - поле Номер свидетельства должно быть заполнено в формате xx-xxxxxxxxx-xx, где x - цифра от 0 до 9; 
    Ф - поле Фамилия должно быть заполнено и должно содержать только русские буквы; 
    И - поле Имя должно содержать только русские буквы; 
    О - поле Отчество должно содержать только русские буквы; 
    БП - поля баллов должны быть не заполнены или содержать только числовые значения от 0 до 100 
    (целые или дробные с одним десятичным знаком после запятой)" 
    </p>
