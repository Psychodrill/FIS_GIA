<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BatchCheckFileFormatResult.aspx.cs" 
    Inherits="Esrp.Web.Certificates.CommonNationalCertificates.BatchCheckFileFormatResult" 
...
<asp:Content ContentPlaceHolderID="cphCertificateContent" runat="server">
    ...
    <div class="txt-in">
    <p><nobr>РЯ - русский язык;</nobr> <nobr>М - математика; Ф - физика;</nobr> <nobr>Х - химия;</nobr> 
    <nobr>Б - биология;</nobr> <nobr>ИР - история России;</nobr> <nobr>Г - география;</nobr> 
    <nobr>АЯ - английский язык;</nobr> <nobr>НЯ - немецкий язык;</nobr> <nobr>ФЯ - французский язык;</nobr> 
    <nobr>О - обществознание;</nobr> <nobr>Л - литература;</nobr> <nobr>ИЯ - испанский язык;</nobr> 
    <nobr>И - информатика.</nobr></p>
    <p>Коды ошибок:
    <nobr>П - неверное число полей, поля должны разделяться символом "%'; </nobr>
    <nobr>НС - поле Номер свидетельства должно быть заполнено в формате xx-xxxxxxxxx-xx, где x - цифра от 0 до 9; </nobr>
    <nobr>Ф - поле Фамилия должно быть заполнено и должно содержать только русские буквы; </nobr>
    <nobr>И - поле Имя должно содержать только русские буквы; </nobr>
    <nobr>О - поле Отчество должно содержать только русские буквы;</nobr> 
    <nobr>БП - поля баллов должны быть не заполнены или содержать только числовые значения от 0 до 100 </nobr>
    <nobr>(целые или дробные с одним десятичным знаком после запятой)" </nobr>
    </p>
    ...
    <asp:DataGrid runat="server" id="dgResultsList"
           OnPageIndexChanged="dgResultsList_PageIndexChanged"
            ...
            CssClass="table-th-border table-th"
             ...
             AllowPaging="True" PagerStyle-Position="Top" PagerStyle-HorizontalAlign="Left" PagerStyle-Mode="NumericPages" PagerStyle-ForeColor="#0066FF" PagerStyle-PageButtonCount="10">
