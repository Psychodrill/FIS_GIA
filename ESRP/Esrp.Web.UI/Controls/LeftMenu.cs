...
namespace Esrp.Web.Controls
{
    ...
    public class LeftMenu : System.Web.UI.Control
    {
        ...
        private string mItemTemplate = "<li class=\"{0}\"><a href=\"{1}\" title=\"{2}\" {3}>{2}</a></li>\r\n";
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            foreach (SiteMapNode node in nodes)
            {
                ...
                if (node == selectedNode)
                    cssClass = "select";
                writer.Write(mItemTemplate, cssClass, node.Url, node.Title, node.Description,node["target"]==null?"":"target='"+node["target"]+"'");
            }
