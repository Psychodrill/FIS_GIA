...
namespace Esrp.Web.Controls
{
    public partial class OperatorPanel : System.Web.UI.UserControl
    {
        ...
        {
            return ConfigurationManager.ConnectionStrings["Esrp.Core.Properties.Settings.EsrpConnectionString"].ToString();
        }
        ...
        private void GetUserInfo(string userLogin, string operatorLogin)
        {
			...
			//cmd.Parameters.Add(new SqlParameter("UserKey", SqlDbType.VarChar, 255));
        ...
        private void GetNewUser(string operatorLogin)
        {
            ...
            if (cmd.Parameters["UserID"].Value != DBNull.Value)
            {
                ...
                Response.Redirect("./View"+ (UserKey??"") +".aspx?login=" + userLogin);
            }
            else
            {
                // пользователей нет - переход на список
				Response.Redirect("./List" + (UserKey ?? "") + ".aspx");
            }
        }
