...
namespace Esrp.Web.UrlShortener
{
    ...
    public static class ShortUrlUtils
    {
        ...
        public static string UniqueShortUrl()
        {
            ...
            if (XmlStorage.GetRealUrl(short_url)==null)
            {
                return short_url;
            }
            else
        ...
        public static string GenerateShortUrl(string longUrl)
        {
            ...
            Container oShortUrl; 
            if ((oShortUrl = XmlStorage.GetShortUrl(HttpUtility.UrlDecode(longUrl))) == null)
            {
                oShortUrl = new Container();
                oShortUrl.RealUrl = HttpUtility.UrlDecode(longUrl);
                oShortUrl.ShortenedUrl = UniqueShortUrl();
                oShortUrl.CreateDate = DateTime.Now;
                oShortUrl.CreatedBy = HttpContext.Current.Request.UserHostAddress;

                AddUrlToDatabase(oShortUrl);
            }
            return oShortUrl.ShortenedUrl;
        }
        public static void AddUrlToDatabase(Container oShortUrl)
        {
            XmlStorage.AddShortUrl(oShortUrl);
        ...
        {
            Container oShortUrl = XmlStorage.GetRealUrl(internal_url) ;
            if (oShortUrl == null)
