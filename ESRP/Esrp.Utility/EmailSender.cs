    ...
    public class EmailSender
    {
       ...
       // public SmtpParams SmtpParams;
           ...
           // edited 10.11.2017 by akopylov
            ...
            catch (SmtpFailedRecipientsException ex)
            {
                for (int i = 0; i < ex.InnerExceptions.Length; i++)
                {
                    ...
                    {
                        //textBox2.Text = "Delivery failed - retrying in 5 seconds.";
                        System.Threading.Thread.Sleep(5000);
                        mail.Send(msg);
                    }
                    else
                    {
                        //textBox2.Text = "Failed to deliver message to {0}" +
                        LogManager.Error(ex.InnerExceptions[i].FailedRecipient); 
                    }
                }
            }
            catch (Exception ex)
            {
                //textBox2.Text = "Exception caught in RetryIfBusy(): {0}" +
                LogManager.Error(ex);
            }
            finally
            {
        ...
        private static SmtpClient CreateSmtpClient()
        {
            ...
            Result.EnableSsl = Esrp.Utility.Properties.Settings.Default.MailClient_EnableSSL;
            return Result;
        }
