IF (EXISTS(
    ...
    alter database CheckEge set single_user with rollback immediate
go
...
IF (EXISTS(
    ...
    drop database CheckEge
go

/*==============================================================*/
/* Database: CheckEge                                           */
/*==============================================================*/
create database CheckEge
go
...
select @CurrentUser = user_name()
execute sp_addextendedproperty N'MS_Description', 
   N'Является ли сочинением/изложением',
   N'user', @CurrentUser, N'table', N'dat_Subjects', N'column', N'IsComposition'
go
...
select @CurrentUser = user_name()
execute sp_addextendedproperty N'MS_Description', 
   N'Является ли базовой математикой',
   N'user', @CurrentUser, N'table', N'dat_Subjects', N'column', N'IsBasicMath'
go
...
select @CurrentUser = user_name()
execute sp_addextendedproperty N'MS_Description', 
   N'Является ли сочинением, по которому вузы могут выгружать бланки',
   N'user', @CurrentUser, N'table', N'dat_Subjects', N'column', N'IsCompositionWithLoadableBlanks'
go
...
select @CurrentUser = user_name()
execute sp_addextendedproperty N'MS_Description', 
   N'Является ли устным псевдопредметом',
   N'user', @CurrentUser, N'table', N'dat_Subjects', N'column', N'IsOral'
go
...
select @CurrentUser = user_name()
execute sp_addextendedproperty N'MS_Description', 
   N'Отображаемое для участника имя предмета в случае, когда оно отличается от SubjectName',
   N'user', @CurrentUser, N'table', N'dat_Subjects', N'column', N'SubjectDisplayName'
go
