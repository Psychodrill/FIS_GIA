<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<GVUZ.Web.ViewModels.OrderOfAdmission.ApplicationSelectOrderViewModel>" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="PageTitle" runat="server">
    Включение заявления в приказ</asp:Content>
...
<asp:Content ID="Content4" ContentPlaceHolderID="MainContent" runat="server">
    <div id="applicationSelectOrderListContainer" style="display: none" class="divstatement notabs">
        <div style="padding: 8px; padding-left: 0">
            <% using (Html.BeginForm("CreateOrderOfAdmission", "OrderOfAdmission", FormMethod.Post, new { id = "createOrderForm" }))
               {
                   for (int i = 0; i < Model.ApplicationsId.Length; i++)
                   { %>
            <input type="hidden" name="<%= string.Format("applicationId[{0}]", i) %>" value="<%= Model.ApplicationsId[i] %>" />
            <%} %>
        ...
        <table class="gvuzDataGrid tableStatement2" cellpadding="3">
            ...
            <thead>
                <tr>
                    ...
                    <%--  Номер приказа --%>
                    <th style="text-align: center">
                        <a data-sort="OrderNumber">
                            <%= Html.LabelTextFor(x => x.RecordInfo.OrderNumber) %></a>
                    </th>
                    <%--  Уровень образования --%>
                    <th style="text-align: center">
        ...
        <div style="padding: 8px; padding-left: 0">
            <% using (Html.BeginForm("MultiIncludeToOrder", "OrderOfAdmission", FormMethod.Post, new { id = "multiIncludeToOrderForm" }))
               {
                   for (int i = 0; i < Model.ApplicationsId.Length; i++)
                   { %>
            <input type="hidden" name="<%= string.Format("applicationId[{0}]", i) %>" value="<%= Model.ApplicationsId[i] %>" />
            <%} %>
            <input type="hidden" name="orderIdField" id="orderIdField" value="0" />
    ...
    <script type="text/javascript">
        ...
        function applicationSelectOrderListLoaded() {
            ...
            ApplicationSelectOrderListViewModel.prototype.reload = function(callback) {
                ...
                //doPostAjax('<%= Url.Action("LoadApplicationSelectOrderRecords", "OrderOfAdmission") %>', ko.toJSON(queryModel, ), function (result)
                queryModel.Filter.SelectedStage = ko.utils.unwrapObservable(this.filter.SelectedStage);
                doPostAjax('<%= Url.Generate<OrderOfAdmissionController>(x => x.LoadApplicationSelectOrderRecords(null, null)) %>', JSON.stringify({ model: queryModel, applicationsId: JSON.parse('<%= Html.Serialize(Model.ApplicationsId) %>') }), function(result) { 
            ...
            ApplicationSelectOrderListViewModel.prototype.actions = {
                ...
                selectOrder: function() {
                    if ($('input:radio[name=group1]:checked').text() != "") {
                        if (<%=Model.ApplicationId == 0 ? "false": "true" %>) {
                            window.location = '<%= Url.Generate<OrderOfAdmissionController>(x => x.IncludeToOrder(null,null))%>?ApplicationID=' + <%=Model.ApplicationId%> + '&OrderId=' + $('input:radio[name=group1]:checked').text();
                        } else {
                            var orderId = $('input:radio[name=group1]:checked').text();
                            $('#multiIncludeToOrderForm').find('#orderIdField').val(orderId);
                            $('#multiIncludeToOrderForm').submit();
                        }
                    } else {
