<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" 
		 Inherits="System.Web.Mvc.ViewPage<GVUZ.Web.ViewModels.Administration.Catalogs.QualificationsViewModel>" %>
...
<asp:Content ID="Main" ContentPlaceHolderID="MainContent" runat="server">
	...
	<div class="divstatement">
		...
		<div id="content">
			...
			<p class="left10 navigation" style="margin-bottom:0px;"><a href="#" id="btnAddNewTop" class="add">Добавить квалификацию/степень</a></p>

			<table class="gvuzDataGrid">
				<thead>
					<tr>
						<th>
							<span class="linkSumulator" onclick="doSort(this, 2)"><%= Html.LabelFor(x => x.QuaDescr.Name) %></span>
						</th>
						<th>
							<span class="linkSumulator" onclick="doSort(this, 3)"><%= Html.LabelFor(x => x.QuaDescr.Code) %></span>
						</th>
						<th id="thAction" style="width:1%;">
							<%= Html.LabelFor(x => x.QuaDescr.QualificationID) %>
						</th>
					</tr>
				</thead>
				<tbody>
				...
				<tfoot>
					<tr>
						<td colspan='2' class="navigation">
							<a href="#" id="btnAddNewBottom" class="add">Добавить квалификацию/степень</a>
						</td>
					</tr>
				</tfoot>
			</table>
	...
	<script language="javascript" type="text/javascript">
		...
		var gridItems = JSON.parse('<%= Html.Serialize(Model.Qualifications) %>')
		...
		function addItem($trBefore, item) {
			...
			$trBefore.before('<tr itemID="' + item.QualificationID + '" class="' + className + '">'
		...
		function updateData() {
			...
			doPostAjax('<%= Url.Generate<AdministrationController>(x => x.GetQualifications(null)) %>', JSON.stringify(model), function (data) {
				if (!addValidationErrorsFromServerResponse(data, false)) {
					gridItems = data.Data.Qualifications
					fillGrid()
		...
		var doAddEdit = function (navUrl, postData, callback) {
			...
			doPostAjax(navUrl, postData, function (data) {
				...
				jQuery('#dialog').dialog({
					modal: true,
					width: 550,
					title: (postData == '' ? "Добавление" : "Редактирование") + " квалификации/степени",
					buttons:
							{
		...
		jQuery(document).ready(function () {
			...
			jQuery('#btnAddNewTop,#btnAddNewBottom').click(function () {
				doAddEdit('<%= Url.Generate<AdministrationController>(x => x.AddQualification()) %>', '',
			...
			jQuery('.btnEdit').live('click', function () {
				...
				doAddEdit('<%= Url.Generate<AdministrationController>(x => x.EditQualification(null)) %>', 'id=' + itemID,
			...
			jQuery('.btnDelete').live('click', function () {
				var $tr = jQuery(this).parents('tr:first');
				var itemID = $tr.attr('itemID')
				confirmDialog('Вы действительно хотите удалить квалификацию/степень?', function () {
					doPostAjax('<%= Url.Generate<AdministrationController>(x => x.DeleteQualification(null)) %>', 'id=' + itemID, function (data) {
