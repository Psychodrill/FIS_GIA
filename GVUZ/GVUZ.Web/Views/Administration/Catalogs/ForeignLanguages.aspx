<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" 
	Inherits="System.Web.Mvc.ViewPage<GVUZ.Web.ViewModels.Administration.Catalogs.ForeignLanguagesViewModel>" %>
...
<asp:Content ID="Main" ContentPlaceHolderID="MainContent" runat="server">
	...
	<div class="divstatement">
		...
		<div id="content">		
			...
			<p class="left10 navigation" style="margin-bottom:0px;"><a href="#" id="btnAddNewTop" class="add">Добавить иностранный язык</a></p>
			
			<table class="gvuzDataGrid">
				<thead>
					<tr>
						<th>
							<span class="linkSumulator" onclick="doSort(this, 2)"><%= Html.LabelFor(x => x.LangDescr.Name) %></span>
						</th>
						<th id="thAction" style="width:1%;">
							<%= Html.LabelFor(x => x.LangDescr.LanguageID) %>
						</th>						
					</tr>					
				</thead>
				<tbody>
				...
				<tfoot>
					<tr>
						<td colspan='2' class="navigation">
							<a href="#" id="btnAddNewBottom" class="add">Добавить иностранный язык</a>
						</td>
					</tr>
				</tfoot>
			</table>
	...
	<script language="javascript" type="text/javascript">
		...
		var gridItems = JSON.parse('<%= Html.Serialize(Model.ForeignLanguages) %>')
		...
		function addItem($trBefore, item) {					
			...
			$trBefore.before('<tr itemID="' + item.LanguageID + '" class="' + className + '">'
		...
		function updateData() {
			...
			doPostAjax('<%= Url.Generate<AdministrationController>(x => x.GetLanguages(null)) %>', JSON.stringify(model), function (data) {
				if (!addValidationErrorsFromServerResponse(data, false)) {
					gridItems = data.Data.ForeignLanguages
					fillGrid()
		...
		var doAddEdit = function (navUrl, postData, callback) {
			...
			doPostAjax(navUrl, postData, function (data) {
				...
				jQuery('#dialog').dialog({
					modal: true,
					width: 500,
					title: (postData == '' ? "Добавление" : "Редактирование") + " иностранного языка",
					buttons:
							{
		...
		jQuery(document).ready(function () {
			...
			jQuery('#btnAddNewTop,#btnAddNewBottom').click(function () {
				doAddEdit('<%= Url.Generate<AdministrationController>(x => x.AddForeignLanguage()) %>', '',
			...
			jQuery('.btnEdit').live('click', function () {
				...
				doAddEdit('<%= Url.Generate<AdministrationController>(x => x.EditForeignLanguage(null)) %>', 'id=' + itemID,
			...
			jQuery('.btnDelete').live('click', function () {			
				var $tr = jQuery(this).parents('tr:first');
				var itemID = $tr.attr('itemID')							
				confirmDialog('Вы действительно хотите удалить язык?', function () {
					doPostAjax('<%= Url.Generate<AdministrationController>(x => x.DeleteForeignLanguage(null)) %>', 'id=' + itemID, function (data) {
